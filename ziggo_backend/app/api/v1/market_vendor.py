...
async def get_daily_earnings(
    date: str,
    db: AsyncSession = Depends(get_db),
    user: User = Depends(require_role("market_owner")),
):
    ...
    for o in orders:
        total_orders += 1
        is_delivered = o.status == MarketOrderStatus.DELIVERED
        is_cancelled = o.status == MarketOrderStatus.CANCELLED
        ...
        for oi in order_items:
            pq = await db.execute(select(Product).where(Product.id == oi.product_id))
            product = pq.scalars().first()
            item_name = product.name if product else "Removed Product"
