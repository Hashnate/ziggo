        ...
        enriched.append(
            {
                ...
                "pickup_fee": float(v.pickup_fee) if v.pickup_fee is not None else 70.0,
...
async def get_my_market_order_details(
    ...
    conditions = [MarketOrder.id == order_id]
    if customer_id:
        conditions.append(MarketOrder.customer_id == customer_id)
    if driver_id:
        conditions.append(MarketOrder.driver_id == driver_id)

    oq = await db.execute(select(MarketOrder).where(*conditions))
...
async def update_market_order_status(
    ...
    q = await db.execute(
        select(MarketOrder)
        .options(joinedload(MarketOrder.vendor))
    ...
    if new_status == MarketOrderStatus.OUT_FOR_DELIVERY:
        ...
        driver_earnings, _ = _market_split(order)
        if order.driver_id:
            ...
            if drv:
                ...
                from ...models import DriverPayout, SystemSettings, Booking, BookingStatus, FoodOrder, FoodOrderStatus
        ...
        # Check and deactivate market vendor if outstanding commission exceeds limit
        from ...services.finance_service import check_and_deactivate_market_vendor
        await check_and_deactivate_market_vendor(db, order.vendor_id)
