...
public class SwiftFlutterContactsPlugin: NSObject, FlutterPlugin, FlutterStreamHandler, CNContactViewControllerDelegate, CNContactPickerDelegate {
    ...
    // Ziggo patch (see packages/flutter_contacts): upstream captured
    // `UIApplication.shared.delegate!.window!!.rootViewController!` inside
    // `register(with:)`. Under the UIScene lifecycle the app delegate owns no window,
    // and plugin registration runs while the storyboard is still instantiating the
    // FlutterViewController, so those unwraps are nil and the app traps on launch.
    // Resolve the view controller lazily instead, when a screen is actually presented.
    private var rootViewController: UIViewController? {
        if #available(iOS 13.0, *) {
            let scenes = UIApplication.shared.connectedScenes.compactMap { $0 as? UIWindowScene }
            let scene = scenes.first(where: { $0.activationState == .foregroundActive }) ?? scenes.first
            if let windows = scene?.windows,
    ...
    // UIKit drops a presentation made on a controller that is already presenting.
    private var presenter: UIViewController? {
        var candidate = rootViewController
