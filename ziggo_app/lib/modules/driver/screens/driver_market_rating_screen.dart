...
class DriverMarketRatingScreen extends StatefulWidget {
  final int orderId;
  const DriverMarketRatingScreen({super.key, required this.orderId});

  @override
  State<DriverMarketRatingScreen> createState() => _DriverMarketRatingScreenState();
}

class _DriverMarketRatingScreenState extends State<DriverMarketRatingScreen> with TickerProviderStateMixin {
  ...
  Future<void> _fetchOrder() async {
    try {
      final resp = await ApiClient.instance.dio.get('/market/orders/${widget.orderId}');
      if (resp.data != null) {
        if (!mounted) return;
        setState(() {
      ...
      debugPrint('Error fetching market order: $e');
    }
  }
      ...
      return const Padding(
        ...
        child: Center(
          child: SizedBox(
            width: 28,
            height: 28,
            child: CircularProgressIndicator(strokeWidth: 2.5, color: _kTeal),
          ),
        ),
      );
    }
