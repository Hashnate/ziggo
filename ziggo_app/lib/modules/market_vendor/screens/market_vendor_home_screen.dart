...
import 'market_vendor_earnings_screen.dart';
...
import 'market_vendor_commission_screen.dart';

class MarketVendorHomeScreen extends StatefulWidget {
  /// When true, the back/close arrow is shown (used when this screen is
  /// pushed from inside the restaurant portal rather than as the auth root).
  final bool embedded;
  const MarketVendorHomeScreen({super.key, this.embedded = false});

  @override
  State<MarketVendorHomeScreen> createState() => _MarketVendorHomeScreenState();
}

class _MarketVendorHomeScreenState extends State<MarketVendorHomeScreen>
    with SingleTickerProviderStateMixin, WidgetsBindingObserver {
  late final TabController _tabs;
  bool _bootstrapped = false;
  bool _historyLoaded = false;
  MarketVendorProvider? _pingSource;
  int _lastPing = 0;
  Timer? _poller;
  ...
  void initState() {
    ...
    // Safety-net poll: WS push can drop silently on Flutter Web when the
    // browser throttles a background tab. Polling every 15s guarantees the
    // pending count stays fresh without relying solely on push.
  ...
  void didChangeAppLifecycleState(AppLifecycleState state) {
    // When the tab regains focus, kick a fresh refresh immediately so the
    // owner doesn't have to wait for the next poll tick.
  ...
  void didChangeDependencies() {
    ...
    if (_pingSource != p) {
      ...
      _pingSource?.noRidersPing.removeListener(_onNoRidersPing);
      _pingSource = p;
      _lastPing = p.newOrderPing.value;
      p.newOrderPing.addListener(_onNewOrderPing);
      p.noRidersPing.addListener(_onNoRidersPing);
    }
  }

  void _onNoRidersPing() {
    final msg = _pingSource?.noRidersPing.value;
    if (msg == null || !mounted) return;
    _pingSource?.noRidersPing.value = null; // consume so it fires per event
  ...
  void _maybeLoadHistory() {
    if (_tabs.index == 2 && !_historyLoaded) {
      _historyLoaded = true;
      context.read<MarketVendorProvider>().loadHistory();
    }
  }
      ...
      const SnackBar(
        content: Row(
          children: [
            ...
            Expanded(
              child: Text(
                'New market order — check the Pending tab',
  ...
  Widget build(BuildContext context) {
    ...
    if (!r.hasProfile) {
      ...
      // "contact admin" screen with the phone number they signed in with.
      return MerchantPendingScreen(
        businessType: 'Market stall',
        icon: Icons.storefront_rounded,
        onClose: widget.embedded ? () => Navigator.pop(context) : null,
      );
    }
    ...
    return Scaffold(
      ...
      body: SafeArea(
        ...
        child: Column(
          children: [
            _Header(
              onLogout: widget.embedded ? () => Navigator.pop(context) : _confirmLogout,
              embedded: widget.embedded,
              onOpenProducts: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const MarketVendorProductsScreen(),
                ),
              ),
              onOpenEarnings: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const MarketVendorEarningsScreen(),
                ),
              ),
              ...
              onOpenCommission: () => Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (_) => const MarketVendorCommissionScreen(),
                ),
              ),
            ),
  ...
  void _openOrder(Map<String, dynamic> order) {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => MarketVendorOrderDetailScreen(order: order),
      ),
    );
  }
}
