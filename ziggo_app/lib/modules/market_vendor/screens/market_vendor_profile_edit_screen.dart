...
class MarketVendorProfileEditScreen extends StatefulWidget {
  const MarketVendorProfileEditScreen({super.key});

  @override
  State<MarketVendorProfileEditScreen> createState() =>
      _MarketVendorProfileEditScreenState();
}

class _MarketVendorProfileEditScreenState
    extends State<MarketVendorProfileEditScreen> {
  ...
  void initState() {
    super.initState();
    final p = context.read<MarketVendorProvider>().profile ??
    ...
    _eta = TextEditingController(text: (p['eta_minutes'] ?? 40).toString());
  }
  ...
  Future<void> _uploadCover(File f) async {
    ...
    final err = await context.read<MarketVendorProvider>().uploadCover(f);
    if (!mounted) return;
  ...
  Future<void> _save() async {
    ...
    setState(() => _busy = true);
    final err = await context.read<MarketVendorProvider>().updateProfile(
