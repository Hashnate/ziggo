...
import 'restaurant_daily_report_screen.dart';


class RestaurantEarningsScreen extends StatefulWidget {
  const RestaurantEarningsScreen({super.key});
...
class _BreakdownGrid extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final delivered = (data['total_delivered'] as num? ?? 0).toInt();
    final orders = (data['total_orders'] as num? ?? 0).toInt();
    final completion = orders == 0
        ? 0
        : ((delivered / orders) * 100).round();
      ...
      onTap: () {
        if (iso.isNotEmpty) {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (_) => RestaurantDailyReportScreen(date: iso),
            ),
          );
        }
      },
