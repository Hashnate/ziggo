...
class _RestaurantAdsScreenState extends State<RestaurantAdsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<RestaurantProvider>().fetchMyAds();
    });
  }
  ...
  Future<void> _uploadAd() async {
    ...
    setState(() => _busy = true);
    final err = await context.read<RestaurantProvider>().uploadAd(
          result['file'] as File,
  ...
  Future<void> _deleteAd(int adId) async {
    ...
    setState(() => _busy = true);
    final err = await context.read<RestaurantProvider>().deleteAd(adId);
    setState(() => _busy = false);
    if (!mounted) return;
    if (err != null) {
      ScaffoldMessenger.of(context).showSnackBar(
